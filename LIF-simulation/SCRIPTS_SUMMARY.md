...
- The voltage-augmented CLI also supports `--dale`, which constrains the fitted model so each presynaptic neuron's outgoing candidate weights share one sign while retaining per-edge magnitudes. The current surrogate-FDR null calibration remains the existing unconstrained circular-shift procedure, so early Dale comparisons should emphasize threshold-free AUC/AP plus false-positive composition diagnostics rather than treating the surrogate-threshold precision/recall as the final verdict.
...
- The voltage-augmented CLI also exposes two opt-in P2 inhibition loss-reshaping knobs, both defaulting to current behavior and leaving the surrogate-FDR machinery unchanged: `--voltage-hyperpol-gamma` (default `0.0`) weights each valid voltage residual by `1 + γ·relu(−vt)` (a hyperpolarization-weighted mean) so below-baseline IPSP bins of the normalized target contribute more gradient (type-free); `--l1-inhibitory-scale` (default `1.0`) scales the L1 penalty on candidates whose presynaptic neuron is inhibitory by ground-truth (**oracle**) outgoing-weight sign, so `0.0` removes L1 sparsity pressure on inhibitory candidates. Saved connectivity npz records `voltage_hyperpol_gamma`, `l1_inhibitory_scale`, and `l1_inhibitory_type_source`.
- The voltage-augmented CLI supports an opt-in **P3 conductance-based synapse** via `--conductance-synapse` (default off = the byte-identical current-based kernel). When on, the per-neuron forward drive becomes `I_syn = gE·(E_E_norm − v) + gI·(E_I_norm − v) + bias` with `gE`/`gI` nonnegative softplus-weighted spike sums routed by **oracle** presynaptic type (sign of outgoing `true_weights`); reversal potentials are read from `lif_simulation.models.LIFNeuron` and mapped into each neuron's normalized voltage frame via the loader's `baseline_medians`/`baseline_scales`. Integration uses an unconditionally-stable backward-Euler step; `|W| = softplus(w)` with the oracle sign. A companion `--seed` (default none) makes a run reproducible for paired A/B comparisons. Saved npz records `conductance_synapse`, `conductance_type_source`, `conductance_e_exc`, `conductance_e_inh`, and `seed`.
