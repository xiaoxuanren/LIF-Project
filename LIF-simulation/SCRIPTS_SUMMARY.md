...
- The voltage-augmented defaults now keep `mask_pre_ms=1.0` but shorten `mask_post_ms` from `5.0` to `2.0` so reset-only saved voltage traces retain more post-spike relaxation while still masking the immediate spike/reset neighborhood.
