...
python -m lif_inference.voltage_augmented_learned_lif_connectivity --session "LIF data/<timestamp>" --connectivity-threshold-mode surrogate_fdr_per_neuron --surrogate-null interval_jitter --jitter-bins 25
python -m scripts.run_voltage_lambda_sweep --session "LIF data/<timestamp>" --lambdas 0.25,0.5,1.0,2.0
python -m scripts.run_jitter_sweep --session "LIF data/<timestamp>" --jitter-bins-list 12,25,40,60
```
...
Both learned-LIF CLIs also expose `--surrogate-null circular_shift|interval_jitter` (default `circular_shift`) and `--jitter-bins` (default `25`) for the surrogate-FDR null. Circular shift rotates each neuron rigidly and destroys population common-input co-activation, so on the **voltage** model its null sits far below the real false-positive floor and the FDR cutoff collapses (precision craters even though AUC/AP are strong). `interval_jitter` resamples each spike within a `--jitter-bins`-wide window, destroying monosynaptic timing below the window while preserving co-activation above it, so the null reproduces the real common-input FP floor — **recommended for voltage runs**. Tune the window with `python -m scripts.run_jitter_sweep`, which reports, per `jitter_bins`, *power* (how far true edges collapse under the null; want small) and *calibration* (null vs observed non-edge floor; want ~1) and recommends the largest window satisfying both.

Recommended voltage configuration: `--surrogate-null interval_jitter --jitter-bins 10` with `surrogate_epochs` matched to the observed model. **Caveat (characterized limitation):** on the voltage model the unsupervised cutoff does *not* reach the oracle frontier. The model's ranking is strong and unchanged (AUC 0.916), but `power` (edge-collapse) and `calibration` (FP-floor match) trade off and cannot both hold — at 1 epoch the null underfits (calibration ~0.33, FP explosion); at matched epochs calibration reaches ~0.95 but the null re-learns true edges (power 0.17→0.75, recall collapses), because the null keeps the real voltage to preserve common input and the real voltage also carries the connectivity signal. Report threshold-free AUC/AP alongside any surrogate operating point; on simulated data the oracle cutoff is the fixed-true-FDR reference. See `docs/surrogate_calibration_summary.md`. Global defaults (`circular_shift`, `surrogate_epochs=1`) are unchanged.
