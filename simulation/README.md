...
The main simulation surface is a conductance-based LIF notebook with a slow hyperpolarization-activated h-current. It is used to generate deterministic noise-free spontaneous activity, stimulus-driven bursting, saved network structure, spike recordings, and voltage traces that feed later analysis and prediction scripts. Saved voltage traces now default to raw full-dt membrane voltage written into chunked external HDF5 sidecars, so the main simulation can keep full-dt traces without building the entire voltage matrix in RAM first. The h-current can be enabled or disabled from the notebook workflow for ablation and validation comparisons.
...
- No-stimulation validation for baseline-driven spontaneous firing with network-burst segmentation
...
Spontaneous and no-stimulation runs force membrane noise to zero and use a frozen, heterogeneous excitatory baseline drive instead:

```powershell
python -m scripts.run_no_stim_validation --baseline-drive-mean 0.11 --baseline-drive-sd 0.05 --exc-weight-scale 0.65
python -m scripts.run_conductance_simulation --mode spontaneous --spontaneous-baseline-mean 0.11 --spontaneous-baseline-sd 0.05 --spontaneous-exc-weight-scale 0.65
```
...
- If the zero-stimulation run is too quiet, tune the frozen baseline mean/spread or recurrent excitatory scale. Do not use membrane noise for the deterministic spontaneous regime.
