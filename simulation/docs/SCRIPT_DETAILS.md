...
- Repository-root legacy CLI wrapper.
- Delegates directly to `lif_inference.learned_lif_connectivity.main()`.
- Kept for CLI compatibility after packaging the implementation.

**`lif_inference/voltage_augmented_learned_lif_connectivity.py`**
- Repository-root legacy CLI wrapper.
- Delegates directly to `lif_inference.voltage_augmented_learned_lif_connectivity.main()`.
- Kept for CLI compatibility after packaging the implementation.
...
| `noise_sigma` | 0.0 | mV | Additive membrane noise; forced off in spontaneous/no-stimulation workflows |
| `i_baseline` | 0.0 | nA-equivalent | Persistent per-neuron baseline drive, assigned once for deterministic spontaneous runs |
...
- The no-stimulation path forces membrane noise to zero, assigns frozen excitatory baseline drive, scales recurrent excitatory weights, and reports burst/inter-burst segmentation metrics.
...
| `--background-noise-sigma` | 0.0 | Stimulus-driven membrane-noise setting; spontaneous mode forces actual noise to zero |
| `--spontaneous-baseline-mean` | 0.11 | Mean frozen excitatory baseline current in spontaneous mode |
| `--spontaneous-baseline-sd` | 0.05 | Standard deviation of frozen excitatory baseline current |
| `--spontaneous-baseline-seed` | None | Seed for the one-time baseline draw; defaults to `--seed` or `0` |
| `--spontaneous-exc-weight-scale` | 0.65 | Recurrent excitatory weight scale in spontaneous mode |
| `--spontaneous-burst-frac-thresh` | 0.12 | Active-neuron fraction threshold for saved spontaneous burst windows |
...
| `--background-noise-sigma` | 0.0 | Legacy option; actual no-stim membrane noise is forced to zero |
| `--baseline-drive-mean` | 0.11 | Mean frozen excitatory baseline current |
| `--baseline-drive-sd` | 0.05 | Standard deviation of frozen excitatory baseline current |
| `--baseline-drive-seed` | None | Seed for the one-time baseline draw; defaults to `--seed` |
| `--exc-weight-scale` | 0.65 | Recurrent excitatory weight scale for no-stim validation |
| `--burst-frac-thresh` | 0.12 | Active-neuron fraction threshold for burst segmentation |
...
4. Require the saved `voltage_sample_rate` to match the requested `dt`; the pipeline stops if they disagree.
5. Normalize each neuron's voltage trace after masking spike neighborhoods and high-voltage peaks.
...
| `mask_post_ms` | 5.0 | Mask duration after each spike-aligned sample neighborhood |
| `peak_threshold_mv` | 15.0 | Mask any saved voltage samples above this threshold |
...
| `--dt` | 1.0 | Spike/voltage bin width in ms |
