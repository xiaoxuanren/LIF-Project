...
The current primary simulation surface is the conductance-based notebook with a slow h-current. It is used to generate deterministic baseline-driven spontaneous activity, stimulus-driven bursting, saved network structure, spike recordings, and raw full-dt membrane-voltage traces for later analysis and model fitting. The notebook now keeps h-current as an explicit toggle so the same code path can be run with or without it for ablation and validation.
...
- `LIF_network_simulation_network_burst_conductance.ipynb` — Monolithic conductance reference notebook retained for parity checks and extraction history.
...
- `LIFNeuron` — Conductance-based LIF neuron with spike-frequency adaptation and an optional slow h-current.
  ...
  - State includes membrane voltage `v`, conductances `g_exc` and `g_inh`, stimulation current `i_ext`, persistent baseline current `i_baseline`, adaptation current `i_adapt`, and h-current state `h_gate` / `i_h`.
  - `i_baseline` is structural frozen drive, not dynamic state; `reset_state()` leaves it unchanged across recordings from the same network.
...
- Zero-stimulation validation cell:
  ...
  - Forces membrane noise to zero, assigns frozen excitatory baseline drive, scales recurrent excitatory weights, and reports burst/inter-burst segmentation metrics
- h-current sag test:
...
### Run a Simulation
Open `LIF_network_simulation_network_burst_conductance.ipynb` and execute the cells in order. Adjust execution parameters in the main run cell near the end of the notebook, including `use_h_current` when you want an ablation run.

### Validate Spontaneous Activity
Run the zero-stimulation validation cell in the conductance notebook. The target regime is noise-free spontaneous activity with a live asynchronous baseline plus intermittent network bursts. Use `use_h_current_test` when you want a matched comparison with h-current disabled.
